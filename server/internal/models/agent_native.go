...
type OutboxDelivery struct {
	...
	DispatchStartedAt *time.Time           `json:"-" gorm:"check:chk_outbox_dispatch_started_at,dispatch_started_at IS NULL OR (destination_type = 'webhook' AND status IN ('processing','succeeded','expired') AND (status <> 'processing' OR (locked_at IS NOT NULL AND (dispatch_started_at = locked_at OR dispatch_started_at > locked_at))))"`
