...
// QuarantineUnsupportedPasswordHashes makes accounts with unverifiable
...
// sessions are revoked. Administrators must complete the normal password-reset
// flow and explicitly reactivate the reviewed human identity.
func QuarantineUnsupportedPasswordHashes(
	...
	if ctx == nil || db == nil {
		return report, errors.New("password quarantine database is unavailable")
	}
...
func revokeHumanSessions(tx *gorm.DB, userID uint, now time.Time) error {
	if err := tx.Model(&RefreshToken{}).
		...
		return fmt.Errorf("revoke quarantined user sessions: %w", err)
	}
	if err := tx.Model(&models.LoginHistory{}).
		...
		return fmt.Errorf("close quarantined user sessions: %w", err)
	}
	return nil
}
