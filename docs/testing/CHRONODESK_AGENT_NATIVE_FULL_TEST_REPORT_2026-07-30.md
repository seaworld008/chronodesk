...
- PR CodeQL Go 分析又发现 6 处平台相关整数窄化和 2 处不可信日志数据流；现统一使用
  原生 `uint` 上界检查，补 32 位边界/交叉编译测试，并让 REST 失败日志只记录白名单
  方法、内部操作标识和固定失败类别，不再写入原始路径、查询或错误文本。
- 可信设备 E2E 曾把 Cookie 查询域硬编码为 `localhost`，导致在 `127.0.0.1` 环境
  误报缺失；现从真实登录响应 URL 派生，并在两个回环域名及完整 27 项序列中通过。
- CI 的脱敏报告目录原以 `.` 开头，`upload-artifact@v7` 默认排除隐藏路径，导致测试
  全通过后仍在制品上传阶段失败；现改用非隐藏安全目录，同时保留上传前凭据扫描。
- 慢速 CI 又暴露工单生命周期 E2E 的 `状态` 选择器会同时命中表头、列宽分隔条、
  SLA 和详情动作；现等待编辑路由与表单就绪，在 `main/form` 内精确定位 combobox，
  并为重试增加逐测试工单清理。专项单轮 3/3、连续 5 轮 15/15 均通过。
